...
public class PergamenaSystem : MonoBehaviour ,Interactable
{
    
    [SerializeField] private String _prompt;

    [SerializeField] private String testoToCanvasPergamena;
    
    [SerializeField] private PergamenaPauseMenu PergamenaPauseMenu;
    
    [SerializeField] private int indexScroll;
    
    [SerializeField] private GameObject CanvasPergamenaPrefab;
    ...
    static string testoCanvasPergamena = "";
    ...
    public void Interact(Interactor interactor)
    {
        ...
        setTextByPergamena(indexScroll);
        
        Debug.Log("//// indexSrollsCollected " + PlayerScript.instance.indexSrollsCollected);
        Debug.Log("//// indexScroll " + indexScroll);

        Debug.Log("//// CheckContains " + PlayerScript.instance.indexSrollsCollected.Contains(indexScroll));
        
        if (!PlayerScript.instance.indexSrollsCollected.Contains(indexScroll))
        {
            Debug.Log("//// non lo contiene");
            PlayerScript.instance.indexSrollsCollected.Add(indexScroll);
        }
        
        myNewGameObject = Instantiate(CanvasPergamenaPrefab, CanvasPergamenaPrefab.transform.position , CanvasPergamenaPrefab.transform.rotation);
    ...
    public void setTextByPergamena(int index)
    {
        
        testoCanvasPergamena = scrolls[index];
        Debug.Log("//// getPergamenaTextSet " + testoCanvasPergamena);
        
    }
    
    public string getTextByPergamena()
    { 
        Debug.Log("//// getPergamenaTexGet " + testoCanvasPergamena);
        
        return testoCanvasPergamena;
    }
